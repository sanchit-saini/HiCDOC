...
The values are then removed for the chromosome (for all conditions and replicates) and kept in 
object@weakBins. 

Then a filter on sparsity can be applied: by chromosome it computes the maximum of sparsity on the interactions matrices (for each matrix of condition x replicate). If a chromosome have a sparsity higher than a given threshold (default to 95%), this chromosome should be removed. 
...
## Load from `hicPro` files

You can use hicPro files. You can create a vector of your matrix files, and a 
vector of the corresponding .bed files. 
Then you can start your script with:

```R
matfiles <- c("link_to_path/matA1.matrix",
              "link_to_path/matA2.matrix",
              "link_to_path/matA3.matrix",
              "link_to_path/matB1.matrix",
              "link_to_path/matB2.matrix",
              "link_to_path/matB3.matrix")
bedfiles <- c("link_to_path/matA1.bed",
              "link_to_path/matA2.bed",
              "link_to_path/matA3.bed",
              "link_to_path/matB1.bed",
              "link_to_path/matB2.bed",
              "link_to_path/matB3.bed")
testhicpro <- HiCDOCDataSetFromHicPro(matfiles, bedfiles, replicates = c("1", "2", "3"), conditions=c("A", "A", "A", "B", "B", "B"))
```
