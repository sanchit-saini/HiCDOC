...
compartments <- compartments %>%
  ...
  filter(grepl("replicate\\..*\\.1", replicate)) %>%
...
concordance %>% 
  ...
  transform(id = as.integer(factor(condition))) %>%
  transform(cond.rep = factor(cond.rep)) -> tmp
  plot_ly(tmp,
          x = ~position,
          y = ~values,
          color = ~cond.rep,
...
subplot(pHeader, pConcordance, nrows = 2, shareX = TRUE, titleX = TRUE, titleY = TRUE, heights=c(0.3, 0.6))
