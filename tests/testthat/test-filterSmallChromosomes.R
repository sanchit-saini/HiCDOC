test_that("filterSmallChromosomes behave as expected", {
    ...
    expect_message(object <- filterSmallChromosomes(object), "Keeping only the chromosomes with 100 bins or more")
    expect_equal(length(object@chromosomes), 2)
    expect_equal(nrow(object@interactions), 86736)
    
    # Filter on 1 chromosome
    expect_message(object <- filterSmallChromosomes(object, 115), "Keeping only the chromosomes with 115 bins or more")
