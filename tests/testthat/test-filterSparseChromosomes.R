test_that("filterSparseChromosomes behaves as expected", {
    ...
    expect_message(filterSparseChromosomes(object), "No chromosome removed (threshold: 95%)", fixed=TRUE)
    ...
    expect_message(object <- filterSparseChromosomes(object, 0.0001), "1 chromosome(s) removed: 17 (threshold : 0.01%)", fixed=TRUE)
