...
#' @keywords internal
#' @noRd
.plotInteractionsGrid <- function(dataplot,
                                  xylim,
                                  transform,
                                  colours,
                                  chromosomeName) {
    plot <-
        ...
        geom_tile(data = dataplot[start1 != start2, ],
...
#' @keywords internal
#' @noRd
.plotInteractionsWrap <- function(dataplot,
                                  xylim,
                                  transform,
                                  colours,
                                  chromosomeName,
...
plotInteractions <- function(object,
    ...
    rowsChromosome <-
        (S4Vectors::mcols(object)$Chr == chromosomeName)
    assayChromosome <-
        SummarizedExperiment::assay(object[rowsChromosome,])
    assayChromosome <- as.data.table(assayChromosome)
    data.table::setnames(assayChromosome,
    ...
    regionsChromosome <-
        as.data.table(InteractionSet::regions(object))
    regionsChromosome <-
        regionsChromosome[seqnames == chromosomeName]
    xylim <-
        c(min(regionsChromosome$start),
          max(regionsChromosome$start))
