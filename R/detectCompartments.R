...
predictAB <- function(object) {
    ...
    compartments <- object@compartments %>%
        dplyr::rename(compartment = value) %>%
        dplyr::left_join(object@diagonalRatios,
                  by = c("chromosome", "condition", "position")) %>%
        dplyr::group_by(chromosome, condition, compartment) %>%
        dplyr::summarize(value = median(value)) %>%
        dplyr::ungroup() %>%
        tidyr::spread(key = compartment,
               value = value,
               fill = 0) %>%
    ...
    object@compartments %<>%
        dplyr::left_join(compartments, by = c("chromosome", "condition")) %>%
        dplyr::mutate(value = factor(if_else(value == A, "A", "B"))) %>%
        dplyr::select(-c(A))
    
    object@concordances %<>%
        dplyr::left_join(compartments, by = c("chromosome", "condition")) %>%
        dplyr::mutate(change = if_else(A == 1, 1,-1)) %>%
        dplyr::mutate(value = change * value) %>%
        dplyr::select(-c(A, change))
    
    object@distances %<>%
        dplyr::left_join(compartments, by = c("chromosome", "condition")) %>%
        dplyr::mutate(cluster = factor(if_else(cluster == A, "A", "B"))) %>%
        dplyr::select(-c(A))
